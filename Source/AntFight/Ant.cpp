...
// the simplest guess of where we will be in the future to help avoid collisions
...
bool AAnt::try_move_on_ground(const FVector& move, float delta_time, bool accel) {
	...
	const FVector tr_start = cur_loc + move * rot_speed_multiplier + vertical_disp;
...
// just push stuff away from each other. somewhat effective.
// TODO: reorganize hierarchy to put capsule as root so collision is handled (will take some work)
void AAnt::resolve_overlaps(float delta_time) {
	...
	for (int i = 0; i < overlapping_ants.Num(); i++) {
		...
		const bool still_overlapping = GetCapsuleComponent()->IsOverlappingComponent(
			Cast<AAnt>(a)->GetCapsuleComponent()
		);
		if (still_overlapping) {
			...
			const float fault_theta_val = FMath::Abs(
				HALF_PI - FMath::Acos(FVector::DotProduct(diff_dir, get_mesh_forward()))
			) / HALF_PI;
			const FVector tr_start = cur_loc + diff_dir * nudge_dist * fault_theta_val * fault_speed_val + vertical_disp;
			
			const float hit_dist = comm::trace_hit_ground(tr_start, tr_start + vertical_tolerance);
			if (hit_dist > 0.0f) {
